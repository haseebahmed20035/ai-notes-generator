...
st.write(
    ...
    "and pictures can be read using OCR."
)
...
def cached_extract_content(filename, file_bytes, enable_ocr):
    return extract_content(filename, file_bytes, enable_ocr=enable_ocr)
...
with st.sidebar:
    ...
    enable_ocr = st.checkbox(
        ...
        value=True,
        help="Turn this off for faster reading if your PDF/PPTX already has selectable text.",
    )
...
uploaded_file = st.file_uploader(
    ...
    type=["pptx", "pdf", "png", "jpg", "jpeg", "webp", "bmp", "tiff"],
)
...
if not combined_text.strip():
    st.warning(
        "We read the file but couldn't find any text. "
        "If your file is scanned or image-based, keep OCR enabled. "
        "If the image is blurry, try a clearer file."
    )
    st.stop()
